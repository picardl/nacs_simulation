...
%bases
NaGnd = atom_basis('Na');
NaD1 = DLine_basis('Na','D1');
NaGnd_cpl = couple_angmom(NaGnd,'s_Na','i_Na','f_Na');
NaD1_cpl = couple_angmom(NaD1,'j_NaD1','i_NaD1','f_NaD1');

%% Define reference point of 0 magnetic field transition
ref_g = [1,1]; %[F,mF] in coupled basis
ref_e = [2,2]; %[F,mF] in coupled basis

[V,D] = eig(NaGnd_cpl.ops.H_Na_hyperfine);
[~,i1] = evec_ind({'f_Na','m_f_Na'},ref_g,NaGnd_cpl,V);
eRef_g = real(D(i1,i1));

[V,D] = eig(NaD1_cpl.ops.H_NaD1_hyperfine);
[~,i2] = evec_ind({'f_NaD1','m_f_NaD1'},ref_e,NaD1_cpl,V);
eRef_e = real(D(i2,i2));
...
trs_e = [3/2,7/2]; % [mJ,mI] in uncoupled basis

[V,D] = eig(NaGnd_cpl.ops.H_Na_hyperfine + NaGnd_cpl.ops.H0_Na_zeeman.*B);
[~,i1] = evec_ind({'f_Na','m_f_Na'},trs_g,NaGnd_cpl,V);
eTrs_g = real(D(i1,i1));

[VD1,DD1] = eig(NaD1.ops.H_NaD1_hyperfine + NaD1.ops.H0_NaD1_zeeman.*B);
[~,i2] = evec_ind({'m_j_NaD1','m_i_NaD1'},trs_e,NaD1,V);
allEs = diag(DD1);
eTrs_e = real(DD1(i2,i2));
...
trs_g = [1,1]; %[F,mF] in coupled basis
...
%transition strength
for j = 1:length(pol)
    allRelStr = zeros(size(NaD1.qnums,1),1);
    for i = 1:length(Fpopns)
        allRelStr = allRelStr + Fpopns(i)*relStr(all_F(i),all_mF(i),NaD1.qnums.j_NaD1(:),NaD1.qnums.m_j_NaD1(:),NaD1.qnums.i_NaD1(:),NaD1.qnums.m_i_NaD1(:),pol(j));
    end
    allEvalRelStr = abs(VD1).^2*allRelStr;
    figure(3)
...
[~,i2] = evec_ind({'m_j_NaD1','m_i_NaD1'},trs_e,NaD1,VD1);
allEs = diag(DD1);
eTrs_e = real(DD1(i2,i2));
nonZeroInd = find(abs(VD1(:,i2))>0);
all_mJ = NaD1.qnums.m_j_NaD1(nonZeroInd);
all_mI = NaD1.qnums.m_i_NaD1(nonZeroInd);
uncpopns = abs(VD1(nonZeroInd,i2)).^2;
