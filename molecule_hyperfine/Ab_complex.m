...
[basis,~,Nchn] = truncate_basis(basis,@(ops) ops.Omega_z,0);
...
%% do the calculation in eigenbasis at chosen R
Rref = 7.4;
ind_ref = find(abs(Rref-R)==min(abs(Rref-R)));
[V,~] = eig(Wint(:,:,ind_ref));

Wint = pagemtimes(V',pagemtimes(Wint,V));
...
out.ops = basis.ops;
out.psi = psi;
save(['data/Ab_' datestr(now,'YYmmDD_HHMMSS') '.mat'],'out')
