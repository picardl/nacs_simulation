...
r.basis = 'aFC';
r.recompute = 2;
r.Jmax = 2;%Max J to inlcude in rotational basis
...
for i = 1:length(BFields)
    ...
    cAll{i} = c3Sigma(BFields(i)*1e-4,'aFC',1,r.Jmax,[3:6],r.f_vib,r);%load(['../data/c_' [strrep(num2str(BFields(i)),'.','p') 'G'] '_vib325THz_' basis '_220124.mat']);
end
...
%     H_dn = vibronic_TDM_X_c * Efield_dn * (c.psi'*rot_TDM_X_c*X.psi);
    ...
    switch basis
        case 'aFC'
            ...
            mj0Ind = find(64 < c.E*1e-9/const.h - 325100 & c.E*1e-9/const.h - 325100 < 74);
        case 'aUC'
    ...
    tdmUpSum(i) = sqrt(sum(H_up.^2));
    tdmUpMax(i) = max(max(abs(H_up)));
