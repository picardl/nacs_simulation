...
T1q_X_perp = sphten([1 1 0]/sqrt(2));
...
switch basis
    case 'aFC'
        ...
        rot_TDM_X_perp_components = operator_matrix(@transition_dipole_case_aFC,...
            {c.qnums(:,~c_ignore),X.qnums(:,~X_ignore)},{'eta','J','Omega','I','F','m_F'},p,T1q_X_perp);
    case {'aUC','aIC'}
        ...
        rot_TDM_X_perp_components = operator_matrix(@transition_dipole_case_a,...
            {c.qnums(:,~c_ignore),X.qnums(:,~X_ignore)},{'eta','J','Omega','m_J'},p,T1q_X_perp);
end
...
rot_TDM_X_perp = (-1).^reshape(p,1,1,3) .* rot_TDM_X_perp_components;
...
rot_TDM_X_perp = permute(rot_TDM_X_perp,[1,2,4,5,3]);
...
psi_c_interp = interp1(c.r,c.psi_vib',X.r,'spline')';
...
vibronic_TDM_X_B = trapz(X.r,X.psi_r.*X1S_B1P_elecTDM_interp.*psi_c_interp(c_B_rows,:),2);
...
% down leg hamiltonian
H_dn = vibronic_TDM_X_B * pagemtimes(pagemtimes(c.psi',rot_TDM_X_perp),X.psi);
