[freqs,survivals,survivals_err] =  loadv0();
offs = 692;
freqs = freqs - offs;
...
r.f_vib = 288694e9; %Freq of transition in Hz. Nearest vibrational level to this value will be selected.
r.pol_up = 'sigpm';
r.pol_frac = 0.9;
...
hold on
plot(ftfreqs,fitfunNoCs([1.1471    0.6942   -0.7794    0.0010],ftfreqs))
lsqfitfunNoCs([1.1471    0.6942   -0.7794    0.0010])
xlabel('Freq [GHz]')
ylabel('Population')
...
%     fitvals = fminsearch(lsqfitfun,[1.1479    0.7105   -0.0253   -0.8488    0.0327],options)
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun,[1.1482    0.7294   -0.0371   -0.8087    0.00264],opstat,'weights',1./survivals_err.^2);
    [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfunNoCs,[1.1482    0.5   -0.8087    0.00264],opstat,'weights',1./survivals_err.^2);

%         fitvals = fminsearch(lsqfitfun2,[1.1483    0.6   -0.6    0.01],options);

end

if 0
    %%
%     fts = [1.1481    0.7248   -0.0158   -0.8286    0.0345];
    r.c3Sigma.Be = fts(1)*1e9*r.h;
    r.c3Sigma.alpha1 = fts(2)*1e9*r.h;
    r.c3Sigma.alpha2 =  fts(3)*1e9*r.h;
    r.pol_up = 0.9;
    out = upleg_hamn_precomp(pre,r,r.B); 
    mJFreq = offToMj1(r,out, beta(4),offs)
end
