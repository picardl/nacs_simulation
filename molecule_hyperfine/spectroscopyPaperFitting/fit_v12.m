[freqs,survivals,survivals_errs] =  loadFiles('names_20191121_201825');% loadFiles({,'names_20191122_002635'});
offs = 306492;
freqs = freqs - offs;
...
r.f_vib =  288694e9;%306490e9; %Freq of transition in Hz. Nearest vibrational level to this value will be selected.
r.pol_up = 'sigpm';
r.pol_frac = 0.9;
...
guess = [r.c3Sigma.Be/(1e9*r.h),r.c3Sigma.alpha1/(1e9*r.h),r.c3Sigma.alpha2/(1e9*r.h),f_off,1e-3,0.9,0.6,0.05];
...
fittp = @(a,b,c,d,e,x) HamnRSC(x,r,pre,a,b,c,d,e,guess(6),guess(7),guess(8))';
...
hold on
plot(ftfreqs,fitfunNoCs([1.0367    0.5458    1.1998    0.0340],ftfreqs))
lsqfitfunNoCs([1.0346    0.2988   0.1888    1.2215    0.2464])
xlabel('Freq [GHz]')
ylabel('Population')

if 0
    %%
    r.c3Sigma.Be = 1.0367*1e9*r.h;
    r.c3Sigma.alpha1 = 0.5458*1e9*r.h;
    r.c3Sigma.alpha2 =  0.05028827*1e9*r.h;
    r.pol_up = 0.9;
    out = upleg_hamn_precomp(pre,r,r.B);
    mJFreq = offToMj1(r,out,  1.1998,offs)
end
...
if 0
    ...
    ftopt = fitoptions('Method', 'NonlinearLeastSquares');
    ftopt.Weights = 1./survivals_errs.^2;
    ftopt.Display = 'iter';
    ftopt.StartPoint = [1   0.7105   -0.0253   0.6    0.5];
    fitfun2 = @(b,x) fitfun([b(1),0.57,0.08,b(2),b(3)],x);
%     lsqfitfun2 = @(b) lsqfitfun_t([b(1),b(2),b(3),b(4),b(5)]);

%     fitvals = fminsearch(lsqfitfun,[1   0.7105   -0.0253   0.6    0.5],options)
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun,[1.0346    0.57   0.08    1.2215    0.05],opstat,'weights',1./survivals_errs.^2);
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun2,[1.0426   1.28    0.0344],opstat,'weights',1./survivals_errs.^2);
        [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfunNoCs,[1.0333    0.57   1.2837    0.0325],opstat,'weights',1./survivals_errs.^2);


%     ft = fit(freqs',survivals',fittp,ftopt)
end
