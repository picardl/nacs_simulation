[freqs,survivals,survivals_errs] =  loadFiles('names_20220111_151019');
offs = 311990.9;
...
r.f_vib = 311990.9e9; %Freq of transition in Hz. Nearest vibrational level to this value will be selected.
r.pol_up = 'sigpm';
r.pol_frac = 0.5;
...
fitfunNoCs = @(b,x) HamnRSC(x,r,pre,1.0150,b(2),0.05028827,b(3),b(4),guess(6),guess(7),guess(8));
...
hold on
plot(ftfreqs,fitfunNoCs([1.0150    0.6571   -0.2688    0.1345],ftfreqs))
lsqfitfunNoCs([0.9653    0.7433      3.0194    0.0204])
xlabel('Freq [GHz]')
ylabel('Population')

if 0
    %%
    r.c3Sigma.Be = 1.0150 *1e9*r.h;
    r.c3Sigma.alpha1 = 0.6571*1e9*r.h;
    r.c3Sigma.alpha2 =  0.05028827*1e9*r.h;
    r.pol_up = 0.9;
    out = upleg_hamn_precomp(pre,r,r.B); 
    mJFreq = offToMj1(r,out, -0.2688,offs)
end
...
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun2,[0.6   0.18    -0.09    0.036],opstat,'weights',1./survivals_errs.^2);
    [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfunNoCs,[1.0150   0.3969  -0.1061    0.1041],opstat,'weights',1./survivals_errs.^2);
