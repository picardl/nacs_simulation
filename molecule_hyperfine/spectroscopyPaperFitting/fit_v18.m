[freqs,survivals,survivals_errs] =  loadFiles('names_20220113_180817');
offs = 314699.2;
...
r.f_vib = 314699.2e9; %Freq of transition in Hz. Nearest vibrational level to this value will be selected.
r.pol_up = 'sigpm';
r.pol_frac = 0.5;
...
fitfunNoCs = @(b,x) HamnRSC(x,r,pre,1.0034,b(1),0.05028827,b(2),b(3),guess(6),guess(7),guess(8));
...
hold on
plot(ftfreqs,fitfunNoCs([0.3984    1.2065    0.4683],ftfreqs))
% lsqfitfun([1.0035   0.2660    0.1230    1.3243    0.0265])
xlabel('Freq [GHz]')
ylabel('Population')

if 0
    %%
    r.c3Sigma.Be = 1.0034*1e9*r.h;
    r.c3Sigma.alpha1 = 0.3984*1e9*r.h;
    r.c3Sigma.alpha2 =  0.05028827*1e9*r.h;
    r.pol_up = 0.9;
    out = upleg_hamn_precomp(pre,r,r.B); 
    mJFreq = offToMj1(r,out,1.2065,offs)
end
...
if 0
    ...
    fitfun2 = @(b,x) fitfun([1.003,b(1),b(2),b(3),b(4)],x);
%     lsqfitfun2 = @(b) lsqfitfun_t([b(1),b(2),b(3),b(4),b(5)]);

%     fitvals = fminsearch(lsqfitfun,[1   0.7105   -0.0253   0.6    0.5],options)
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun,[1.0035    0.5735    0.0501   1.3   0.0435],opstat,'weights',1./survivals_errs.^2);
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun2,[0.5735    0.0501   1.3   0.0435],opstat,'weights',1./survivals_errs.^2);
    [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfunNoCs,[0.5735   1.3   0.0435],opstat,'weights',1./survivals_errs.^2);
