% [freqs,survivals,survivals_errs] =  loadFiles({'names_20220126_005424','names_20220126_082139',...
...
r.f_vib = 320010.6e9; %Freq of transition in Hz. Nearest vibrational level to this value will be selected.
r.pol_up = 'sigp';
r.pol_frac = 1;0.5;
...
guess = [r.c3Sigma.Be/(1e9*r.h),r.c3Sigma.alpha1/(1e9*r.h),r.c3Sigma.alpha2/(1e9*r.h),f_off,1e-3,1,mean(survivals(end-20:end))-min(survivals),min(survivals)];
fitfun = @(b,x) HamnRSC(x,r,pre,b(1),b(2),b(3),b(4),b(5),guess(6),guess(7),guess(8));
% lsqOnlyCs = @(b) sum((survivals - HamnRSC(freqs,r,pre,b(1),b(2),0,b(3),b(4),guess(6),guess(7),guess(8))).^2);
...
lsqfitfunNoCs =@(b) sum((survivals - HamnRSC(freqs,r,pre,b(1),b(2),0.05028827,b(3),b(4),guess(6),guess(7),guess(8))).^2);
lsqfitfunNoNa =@(b) sum((survivals - HamnRSC(freqs,r,pre,b(1),0.4429065,b(2),b(3),b(4),guess(6),guess(7),guess(8))).^2);

lsqOnlyCs =@(b) sum((survivals - HamnRSC(freqs,r,pre,1.0055,0.3179,b,2.5488,0.0083,guess(6),guess(7),guess(8))).^2);
...
hold on
% plot(ftfreqs,fitfun([0.9797    0.4181   0.050288    2.6877    0.0074],ftfreqs))
% plot(ftfreqs,fitfunNoCs([0.9814    0.4126    2.6866    0.0084],ftfreqs))
% plot(ftfreqs,fitfunNoCs([1.0055    0.3179    2.5488    0.0083],ftfreqs))
plot(ftfreqs,fitfunNoCs([0.9814    0.63    2.6866    0.0084],ftfreqs))

% lsqfitfun([0.9817    0.3416    0.0901    2.7077    0.0073])
lsqfitfunNoCs([0.9814    0.4126    2.6866    0.0084])
...
if 0
    %%
    r.c3Sigma.Be = 0.9814*1e9*r.h;
    r.c3Sigma.alpha1 = 0.4126*1e9*r.h;
    r.c3Sigma.alpha2 =  0.05028827*1e9*r.h;
    r.pol_up = 0.9;
    out = upleg_hamn_precomp(pre,r,r.B); 
    mJFreq = offToMj1(r,out,2.6866,offs)
end
...
%     fitvals = fminsearch(lsqfitfunNoCs,[0.9834    0.2979   2.7954    0.0082],options)
%       fitvals = fminsearch(lsqfitfunNoCs,[0.9785   0.4429065   2.6846    0.0188],options)
%       fitvals = fminsearch(lsqfitfunNoNa,[0.9817    0.04    2.7077    0.0073],options)
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun,[0.9837    0.2771    0.1204    2.7996    0.0093],opstat,'weights',1./survivals_errs.^2);
    [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfunNoCs,[0.9837    0.5    2.7996    0.0093],opstat,'weights',1./survivals_errs.^2);
