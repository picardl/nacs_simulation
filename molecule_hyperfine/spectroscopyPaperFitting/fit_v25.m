[freqs,survivals,survivals_errs] =  loadFiles('names_20220120_194641',1:140);
offs = 323874.4;
...
r.f_vib = 323874.4e9; %Freq of transition in Hz. Nearest vibrational level to this value will be selected.
r.pol_up = 'sigpm';
r.pol_frac = 0.5;
...
hold on
plot(ftfreqs,fitfunNoCs([0.9863    0.4606    2.5451    0.0384],ftfreqs))
lsqfitfunNoCs([0.9863    0.4606    2.5451    0.0384])
xlabel('Freq [GHz]')
ylabel('Population')
...
%     fitvals = fminsearch(lsqfitfun,[0.9579    0.3566    0.01  2.3350    0.0211],options)
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun,[0.9837    0.2771    0.1204    2.5    0.0093],opstat,'weights',1./survivals_errs.^2);
    [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfunNoCs,[0.9837    0.2771   2.5    0.0093],opstat,'weights',1./survivals_errs.^2);

%     fitvals = fminsearch(lsqfitfunNoCs,[0.9630    0.3373     2.4    0.02],options)
%         fitvals = fminsearch(lsqfitfunNoNa,[0.9630    0.09     2.4    0.02],options)
