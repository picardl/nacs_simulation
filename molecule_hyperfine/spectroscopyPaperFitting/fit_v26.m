...
% loadFiles('names_20220108_170735',[1:55]);%loadFiles({'names_20200623_101608','names_20200623_152639'});
offs = 325125;
freqs = freqs - offs;
...
r.B = 866*1e-4; %B Field in Gauss. updated FR in model is at 864.1
...
guess = [r.c3Sigma.Be/(1e9*r.h),r.c3Sigma.alpha1/(1e9*r.h),r.c3Sigma.alpha2/(1e9*r.h),f_off,1e-3,r.pol_frac,mean(survivals(end-10:end))-min(survivals),min(survivals)];
...
hold on
% plot(ftfreqs,fitfunNoCs([0.9530    0.3058   -1.9490    0.0748],ftfreqs))
plot(ftfreqs,fitfunNoCs([0.9613    0.3249   -1.9110    0.0740],ftfreqs))
lsqfitfunNoCs([0.9530    0.3058   -1.9490    0.0748])
xlabel('Freq [GHz]')
ylabel('Population')
...
%     fitvals = fminsearch(lsqfitfun,[1   0.4    0.6    0.5],options)
%     [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfun,[0.9511    0.3685    0.0212   -1.9675    0.02],opstat,'weights',1./survivals_errs.^2);
    [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(freqs,survivals,fitfunNoCs,[0.9530    0.5   -1.9490    0.0748],opstat,'weights',1./survivals_errs.^2);
