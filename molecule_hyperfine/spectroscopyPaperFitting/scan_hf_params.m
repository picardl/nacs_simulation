...
paramRange = [0.4:0.025:1]*1e9*r.h;

allfitparamsv0 = cell(1,length(paramRange));
allfitparamsv26 = cell(1,length(paramRange));
allfitfunsv0 = cell(1,length(paramRange));
allfitfunsv26 = cell(1,length(paramRange));

allfitparamsv22 = cell(1,length(paramRange));
allfitfunsv22 = cell(1,length(paramRange));
for i = 1:length(paramRange)
    
% r.c3Sigma.Be = paramRange(i);%1.15*1e9*r.h;
% r.c3Sigma.alpha2 = paramRange(i);
% r.c3Sigma.gS  = paramRange(i);
r.c3Sigma.alpha1  = paramRange(i);
...
resids22(i) = mean(residualsv22.^2);
allfitparamsv22{i} = fitparamsv22;
allfitfunsv22{i} = fitfunv22;
...
%     xlabel('Freq [GHz]')
    ...
    hold on
    plot(fPlot,allfitfunsv22{ind}(allfitparamsv22{ind},fPlot),'color','black');
    legend({'Data','Model fit'})
    xlabel('Freq [GHz]')
    ylabel('Survival')
    title('v = 22')
