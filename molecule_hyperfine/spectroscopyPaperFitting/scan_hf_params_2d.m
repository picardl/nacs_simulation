...
resids0 = [];
resids26 = [];
% paramRange = [1.1:0.01:1.2]*1e9*r.h;
% paramRange = [-0.1:0.02:0.1]*1e9*r.h;
% paramRange = 2.0023 + [-0.05:0.005:0];
paramRange1 = [0.4:0.025:0.8]*1e9*r.h;
paramRange2 = [0.9:0.05:1.2]*1e9*r.h;

allfitparamsv0 = cell(length(paramRange1),length(paramRange2));
allfitparamsv26 = cell(length(paramRange1),length(paramRange2));
allfitfunsv0 = cell(length(paramRange1),length(paramRange2));
allfitfunsv26 = cell(length(paramRange1),length(paramRange2));

allfitparamsv22 = cell(length(paramRange1),length(paramRange2));
allfitfunsv22 = cell(length(paramRange1),length(paramRange2));

resids0 = zeros(length(paramRange1),length(paramRange2));
resids22 = zeros(length(paramRange1),length(paramRange2));
resids26 = zeros(length(paramRange1),length(paramRange2));

for i = 1:length(paramRange1)
    for j = 1:length(paramRange2)
r.c3Sigma.Be = paramRange2(j);%1.15*1e9*r.h;
% r.c3Sigma.alpha2 = paramRange(i);
% r.c3Sigma.gS  = paramRange(i);
r.c3Sigma.alpha1  = paramRange1(i);

% r.f_vib = 288690;
% r.Gamma = r.h*90e6;
% [residualsv0, fitparamsv0,fitcovv0, fitfunv0] = fitHf(freqsv0,v0survivals,v0survivals_err,r,guessv0);
% resids0(i) = mean(residualsv0.^2);
% allfitparamsv0{i} = fitparamsv0;
% allfitfunsv0{i} = fitfunv0;

r.f_vib = 320000;
r.Gamma = r.h*15e6;
r.pol_up = 'sigpm';
[residualsv22, fitparamsv22,fitcovv22, fitfunv22] = fitHf(freqsv22,v22survivals,v22survivals_err,r,guessv22);
resids22(i,j) = mean(residualsv22.^2);
allfitparamsv22{i,j} = fitparamsv22;
allfitfunsv22{i,j} = fitfunv22;

% r.f_vib = 325129;
% r.Gamma = r.h*90e6;
% r.pol_up = 'sigp';
% [residualsv26, fitparamsv26,fitcovv26, fitfunv26] = fitHf(freqsv26,v26survivals,v26survivals_err,r,guessv26);
% resids26(i) = mean(residualsv26.^2);
% allfitparamsv26{i} = fitparamsv26;
% allfitfunsv26{i} = fitfunv26;
    end
end
% 
...
% figure(5)
% clf
% plot(paramRange,resids0)
% hold on
% plot(paramRange,resids26)
% legend({'v = 0','v = 26'})
...
figure(6)
clf
[X,Y] = meshgrid(paramRange1,paramRange2);
surf(X,Y,resids22','FaceColor','interp')
ylabel('Be')
xlabel('alpha1')
c = colorbar();
...
%     hold on
%     plot(fPlot,allfitfunsv0{ind}(allfitparamsv0{ind},fPlot),'color','black');
...
%     xlabel('Freq [GHz]')
    ...
    clf
    subplot(3,1,[1:2])
    fPlot = linspace(freqsv22(1),freqsv22(end),1e4);
    errorbar(freqsv22,v22survivals,v22survivals_err)
    hold on
    plot(fPlot,allfitfunsv22{inds(1),inds(2)}(allfitparamsv22{inds(1),inds(2)},fPlot),'color','black');
    legend({'Data','Model fit'})
    xlabel('Freq [GHz]')
    ylabel('Survival')
    title('v = 22')
...
%     hold on
%     plot(fPlot,allfitfunsv26{ind}(allfitparamsv26{ind},fPlot),'color','black');
%     legend({'Data','Model fit'})
%     xlabel('Freq [GHz]')
%     ylabel('Survival')
%     title('v = 26')
...
function [residuals, fitparams,fitcov, fitfun] = fitHf(freqs,survivals,survivals_err,ramanSet,guess)
    ...
    raman_data = upleg_effective_hamiltonian(ramanSet,ramanSet.B,ramanSet.basis,ramanSet.recompute,ramanSet.Jmax,ramanSet.mtot,ramanSet.f_vib);
