...
rmin = 4.5;
rmax = 10000;
mtot = 5; % total angular momentum to truncate basis
Erange_vs_threshold = [-100e6 0]*c.h/c.hartree;
...
[basis.F1C,basis.UC,basis.change.UC_F1C] = couple_angmom(basis.UC,'i_Na','s_Na','f_Na');
[basis.FC,basis.F1C,basis.change.F1C_FC] = couple_angmom(basis.F1C,'i_Cs','s_Cs','f_Cs');

basis.change.SC_FC = (basis.change.UC_SC)' * basis.change.UC_F1C * basis.change.F1C_FC;

f = setdiff(fields(basis.FC.ops),fields(basis.SC.ops));
for i = 1:numel(f)
    basis.SC.ops.(f{i}) = basis.change.SC_FC * basis.FC.ops.(f{i}) * basis.change.SC_FC';
end
...
figure(3);
clf;
plot(rtest,D'*c.hartree/c.h*1e-9);
ylim([-1 1]*20)
...
save(['data/vm1_' datestr(now,'YYmmDD_HHMMSS') '.mat'],'out')
end